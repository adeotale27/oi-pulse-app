...
OI `/change` no longer 503s when the UI still asks for yesterday’s weekly expiry — it serves the latest chain. 503s are swallowed in the client so the red overlay does not cover the desk. Recharts no longer measures a -1×-1 chart box. Kite `generate_session` and `profile` run off the event loop with timeouts.
