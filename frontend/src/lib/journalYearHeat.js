import { DESK_IDS, emptyDeskPnl, matchSymbolPrefix } from "./universe.js";
...
export function heatmapIndexFromLeg(leg) {
  ...
  if (DESK_IDS.includes(raw)) return raw;
...
function bookedIndexPnl(doc) {
  ...
  if (legs.length) {
    ...
    for (const k of DESK_IDS) acc[k] += Number(ip[k]) || 0;
    for (const [k, v] of Object.entries(ip)) {
      if (!DESK_IDS.includes(String(k).toUpperCase())) other += Number(v) || 0;
    }
  }
  const attributed = DESK_IDS.reduce((s, k) => s + acc[k], 0) + other;
...
function emptyHeat(year) {
  const by_index = Object.fromEntries(DESK_IDS.map((idx) => [idx, Array(12).fill(0)]));
  return {
    year,
    indices: DESK_IDS.slice(),
    by_index,
...
export function overlayMonthOnYearHeat(yearHeat, monthPayload, year, month) {
  ...
  days.forEach((d) => {
    ...
    for (const k of DESK_IDS) idxAcc[k] += Number(ip[k]) || 0;
  ...
  for (const k of DESK_IDS) heat.by_index[k][mi] = idxAcc[k];
  return heat;
}
