...
const vis = { instrument: true, avg: false };
const ids = visibleColumnIds(vis, ["qty", "instrument", "product", "avg"]);
assert.deepEqual(ids, ["instrument"]);
...
const swapped = moveColumn(orig, "pnl", "avg");
assert.equal(swapped[1], "pnl");
assert.ok(swapped.indexOf("avg") > swapped.indexOf("pnl"));
...
assert.equal(loaded[0], "instrument");
