...
assert.equal(listingCountryCode("XETRA"), "DE");
assert.equal(listingFlag("NYSE"), "🇺🇸");
assert.equal(listingFlag({ exchange: "FRA" }), "🇩🇪");
assert.equal(isAdrSessionOpen({ listing_open: true }), true);
assert.equal(isAdrSessionOpen({ listing_open: false, is_market_open: true, display_status: "CURRENT", stale: false }), false);
assert.equal(isAdrSessionOpen({ display_status: "CURRENT", stale: false }), false);
