...
import { CalendarDays, PartyPopper, ChevronDown, ChevronRight } from "lucide-react";
      ...
      <div>
        ...
        {past.length > 0 && (
          <div className="mb-2">
            <button
              type="button"
              data-testid="holidays-past-toggle"
            ...
            {pastOpen && (
              <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                {past.map((h) => <HolidayCard key={h.date} h={h} today={today} />)}
              </div>
            )}
          </div>
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {upcoming.map((h) => <HolidayCard key={h.date} h={h} today={today} />)}
        </div>
      </div>
