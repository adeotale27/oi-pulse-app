...
export default function MarketEventsBadge({ onClick }) {
  ...
  if (!primary) {
    return (
      <div className="rounded-sm border border-slate-200 bg-white px-2.5 py-1 text-[10px] text-slate-500">
        No major events tracked
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full" data-testid="events-badge-wrap">
      <div
        role="button"
        tabIndex={0}
        ...
        className={`w-full rounded-sm border-2 px-2.5 py-1.5 text-left cursor-pointer transition-colors hover:brightness-95 ${cls}`}
      ...
      >
        ...
        <div className="text-xs font-semibold leading-tight" data-testid="events-badge-name">
          {primary.name}
        </div>
      ...
      {open && extras.length > 0 && (
        <div
          ...
          className="absolute right-0 mt-1 w-72 rounded-md border border-slate-200 bg-white shadow-xl z-40 max-h-96 overflow-y-auto"
        >
          <div className="px-3 py-2 border-b border-slate-100 text-[10px] uppercase tracking-widest text-slate-500 flex items-center justify-between">
            ...
            <button className="text-slate-400 hover:text-slate-800" onClick={() => setOpen(false)}>✕</button>
          </div>
