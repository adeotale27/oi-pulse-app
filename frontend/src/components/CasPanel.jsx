...
const GUIDE = (
  <div className="space-y-2 text-[12px] leading-relaxed">
    ...
    <p>
      ...
      Default schedule: NIFTY Mon/Tue, SENSEX Wed/Thu (configurable).
    </p>
  </div>
);
...
export default function CasPanel({ isAdmin = false, isKiteMode = false, onOpenKite }) {
  ...
  // Get the auto trade data for the active index(es)
  const getAutoDataForIndex = (index) => {
    return status?.[index] || {};
  };

  const autoNifty = getAutoDataForIndex("NIFTY");
  const autoSensex = getAutoDataForIndex("SENSEX");

  // For backward compatibility and single index mode, use the primary auto data
  const auto = activeIndex === "NIFTY" || !bseEnabled ? autoNifty :
               activeIndex === "SENSEX" ? autoSensex : autoNifty;
      ...
      {tab === "live" && (
        <>
            ...
            <div className="flex flex-wrap items-end gap-4">
              ...
              {isAdmin && (
                <div>
                  ...
                  <div className="flex items-center gap-2 h-8">
                    <Switch
                      checked={settings.bse_enabled}
                      disabled={!isAdmin || busy}
                      onCheckedChange={(checked) => patchSettings({ bse_enabled: checked })}
                      data-testid="cas-bse-toggle"
                    />
                    <span className="text-[11px] text-slate-700">
                      Enable BSE SENSEX indicative
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-500 mt-1">
                    When enabled, BSE SENSEX will be used on Wed/Thu by default
                  </div>
                </div>
              )}
          ...
          >
            ...
            {/* Auto Trade Tape - show both indices when BOTH is selected */}
            {activeIndex === "BOTH" && bseEnabled ? (
              <>
                <AutoTapeStrip auto={autoNifty} autoLots={autoLots} index="NIFTY" />
                <AutoTapeStrip auto={autoSensex} autoLots={autoLots} index="SENSEX" />
              </>
            ) : (
              <AutoTapeStrip auto={auto} autoLots={autoLots} index={activeIndex} />
            )}

            {isAdmin && autoPaper && !autoLive && (
              <div className="flex flex-wrap items-end gap-2 pt-1 border-t border-slate-100">
                <label className="text-[10px] uppercase tracking-wider text-slate-500">
                  Inject indicative (paper)
                  <input
                    type="number"
                    step="0.05"
                    value={injectValue}
                ...
                <p className="text-[10px] text-slate-500 max-w-md leading-snug">
                  Number = fake indicative. Example: if frozen indicative is 23980, enter 24007 to
...
function AutoTapeStrip({ auto, autoLots, index }) {
  ...
  return (
    <div className="space-y-2" data-testid="cas-auto-tape">
      ...
      >
        {err && (
          <p className="font-semibold break-all" data-testid={`cas-auto-${index.toLowerCase()}-error`}>
            Error: {auto.nse_error ? `${index} ${auto.nse_error}` : auto.reason}
          </p>
        )}
        {!err && auto.nse_fetched_at && (
          <p data-testid="cas-auto-${index.toLowerCase()}-line">
        ...
        {auto.nse_fallback_value != null && (
          <p className="text-amber-800" data-testid={`cas-auto-${index.toLowerCase()}-fallback`}>
        ...
        {(auto.cookies_ok || (auto.cookie_names || []).length > 0) && (
          <p data-testid={`cas-auto-${index.toLowerCase()}-cookies`}>
      ...
      {auto.last_rehearsal?.status && (
        <p className="text-[11px] text-sky-800" data-testid="cas-auto-rehearsal">
          ...
          {auto.last_rehearsal.pre_signal != null
            ? ` · freeze ${fmt(auto.last_rehearsal.pre_signal, 2)}`
            : ""}
          {auto.last_rehearsal.locked_atm != null ? ` · ATM ${auto.last_rehearsal.locked_atm}` : ""}
          {auto.last_rehearsal.indicative != null
            ? ` · print ${fmt(auto.last_rehearsal.indicative, 2)}`
            : ""}
        </p>
      )}
    </div>
