...
export default function AdminControls() {
  ...
  const togglePublic = async () => {
    ...
    try {
      ...
      toast.success(data.open ? "Public access ON — expires at 3:30 PM IST" : "Public access OFF");
      await refresh();
    } catch (e) {
  ...
  return (
    <div className="flex items-center gap-2 pr-2 mr-1 border-r border-slate-200 dark:border-slate-700" data-testid="admin-controls">
        ...
        {menuOpen && (
          ...
          >
            <button
              ...
              onClick={() => { setMenuOpen(false); setGuestsOpen(true); }}
              className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-slate-50"
            >
              <Users className="w-3.5 h-3.5" /> Guest Directory
            </button>
            <button
      ...
      <GuestDirectoryModal open={guestsOpen} onOpenChange={setGuestsOpen} />
