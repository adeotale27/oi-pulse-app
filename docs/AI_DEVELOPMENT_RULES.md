...
8. Do not add dependencies unless the task cannot be done with the current stack.
   For every package or lockfile update, complete the package, dependency, and
   supply-chain risk section in [`CHANGE_CHECKLIST.md`](./CHANGE_CHECKLIST.md):
   review advisories and breaking changes, inspect the full lockfile diff, run
   frozen installation and validation, check protected-data boundaries, and
...
25. Protected live-data rule: do not change OI, OI Change, Open Interest, strike
    math, Positions, broker-book/P&L mapping, Straddle, CAS signals, snapshot
    freshness, expiry selection, or poll semantics unless the administrator
    explicitly approves that exact behavior change. UI polish and safe settings
    changes may proceed; otherwise stop and ask.
26. Before every edit, read [`CHANGE_CHECKLIST.md`](./CHANGE_CHECKLIST.md).
    Major/non-obvious frontend and backend behavior must have a concise
    why/invariant comment. If code changes invalidate a nearby comment, update
    that comment in the same change. Complete the checklist's tests, docs,
    version, diff, and ship steps.
